...
def test_save_and_get_prescription():
    result_dict = {
        ...
        "extracted": {
            ...
            "medications": [
                {"drug_name": "Paracetamol", "dosage": "500mg", "frequency": "1-1-1"}
            ],
        },
    }
...
def test_get_all_prescriptions():
    for i in range(3):
        save_prescription(
            {"extracted": {"patient": {"name": f"Patient {i}"}}}, db_path=TEST_DB
        )
...
def test_search_prescriptions():
    ...
    save_prescription(
        {"extracted": {"medications": [{"drug_name": "Ibuprofen"}]}}, db_path=TEST_DB
    )
...
def test_get_stats():
    save_prescription(
        {"ocr_confidence": 90.0, "extracted": {"patient": {"name": "Alice"}}},
        db_path=TEST_DB,
    )
    save_prescription(
        {"ocr_confidence": 100.0, "extracted": {"patient": {"name": "Bob"}}},
        db_path=TEST_DB,
    )
